      ...
      <div className="step-header">
        {!isCompleted ? (
          ...
          >
            ...
            <span className={`number-h ${size} ${isTextGreen ? 'green-text' : ''}`}>
              {stepNumber}
            </span>
          </div>
        ) : (
          <div className="number-box-h" style={{ width: config.svgSize, height: config.svgSize, minWidth: config.svgSize }}>
        ...
        <div
          ...
          onMouseLeave={() => setIsHovered(false)}
        >
          <div className="question-wrapper-h">
            <span
              className={`label-text-h ${size} no-bg${isTextGreen ? ' green-text' : ''}`}
            >
              {question}
            </span>
            <div className={`question-underline-h ${size}${isActive && !isCompleted ? ' active' : ''}`} />
          </div>

          <div
            className="input-wrapper-h"
            onMouseEnter={() => setIsInputHovered(true)}
            onMouseLeave={() => setIsInputHovered(false)}
          >
            <div className={`underline-h ${size}${isBorderGreen ? ' active' : ''}`} />
            <input
              type="text"
              value={answer}
              ...
              onFocus={handleFocus}
              onBlur={handleBlur}
              className={`answer-input-h ${size}${isTextGreen ? ' green-text' : ''}`}
              style={{
...
export default HorizontalQuestionForm;
